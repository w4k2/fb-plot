...
for res_file in RESULTS:
    ...
    ax.scatter(recall[:, n], precision[:, n], s=40, c="#DDDDDD")

    for b in unique_best:
        ax.scatter(recall[b, n], precision[b, n], s=170, marker="*")
    ...
    for b in unique_best:
        ax.plot(betas[best == b], f_betas[:, b][best == b], lw=3, label=clf_names[b])[0]
    ...
    plt.tight_layout()
    plt.savefig(f"figures/03_{ds_name}.png")
    plt.savefig(f"figures/03_{ds_name}.pdf")
    plt.close()
